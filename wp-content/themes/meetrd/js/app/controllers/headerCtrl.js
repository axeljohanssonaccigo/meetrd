...
headerApp.controller('headerCtrl', function ($scope, headerSvc) {
    ...
    $scope.isRoomPage = location.pathname.search('room') > -1;
    if ($scope.isRoomPage) {
        //Use login button in boking module
        $scope.loginHref = "javascript:;";
    } else {
        $scope.loginHref = "/wp-login-php";
    ...
    $scope.clickBookingLoginButton = function () {
        if ($scope.isRoomPage && (jQuery('#bookingLoginButton a').length > 0)) {
            jQuery('#bookingLoginButton a')[0].click();
        }
    };
});
