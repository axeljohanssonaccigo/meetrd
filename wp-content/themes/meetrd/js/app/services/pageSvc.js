pageApp.service('pageSvc', function ($http) {
	...
	this.registerUser = function(nonce, user){
		...
		return $http({
			method: "GET",
			url: urlPathNameAddOn + '/api/user/register/?username=' + user.username + '&nickname=' + user.nickname + '&email=' + user.email + '&nonce=' + nonce + '&first_name=' + user.firstName + '&last_name=' + user.lastName + '&display_name=' + user.username + '&user_pass=' + user.password + '&description=' + user.biography + '&user_url=' + user.website + '&custom[wpcf-phone]=' + user.phone + '&custom[wpcf-billing-address]="' + user.billingAddress.toString() + '"&show_admin_bar_front=no' + '&insecure=cool'
		});
	};
