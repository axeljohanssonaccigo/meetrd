    ...
    public static function wpcf_access_user_has_cap_filter($allcaps, $caps, $args) 
    {
        ...
        if ( preg_match( $admin_include_caps_list, $args[0]) && !in_array( $args[0], $admin_exclude_caps) ){
            ...
            if ( preg_match( "/(^edit\_|^delete\_|^publish\_)/", $args[0], $res) && $args[0] !== 'edit_comments' ){
               ...
               // Todo for 1.2.8
               // Check how cache
               /* $access_cache_user_has_cap_for_posts_key = md5('access::user_posts' . serialize($caps)  . 'cap' . serialize($args) );
                $cached_caps = wp_cache_get( $access_cache_user_has_cap_for_posts_key, 'access_cache_user_has_cap_for_posts' );
                if ( false === $cached_caps ){
                    ...
                    wp_cache_add( $access_cache_user_has_cap_for_posts_key, $allcaps, 'access_cache_user_has_cap_for_posts' );
                }else{
                    $allcaps = $cached_caps;
                }*/
                return $allcaps;
